...
public:
    ...
    */
    ...
    /**
    * \fn void set_SFP_registers_names(std::map<T_SFPconfigKey, std::string> map, T_SFPName sfp_name)
    * \brief Set SFP registers names
    * \param map : it is a map contain the list of registers names for the SFP config
    * \param sfp_name : it is the SFPs ports name (T_SFPName::SFP1, T_SFPName::SFP2 or T_SFPName::SFP3). Maximum, we have three SFP port to receive images data.
	* \return void
    */
    void set_SFP_registers_names(std::map<T_SFPconfigKey, std::string> map, T_SFPName sfp_name);

    /**
    * \fn T_UDPConfig get_SFP_network_config(T_SFPName sfp_name)
    * \brief Get network configuration for specified SFP output
    * \param sfp_name : it is the SFPs ports name (T_SFPName::SFP1, T_SFPName::SFP2 or T_SFPName::SFP3). Maximum, we have three SFP port to receive images data.
	* \return structure to specify the udp network config.
    */
    T_UDPConfig get_SFP_network_config(T_SFPName sfp_name);

    /**
    * \fn void set_SFP_network_config(T_UDPConfig udp_config, T_SFPName sfp_name)
    * \brief Set network configuration for specified SFP output
    * \param udp_config : structure to specify the udp network config.
    * \param sfp_name : it is the SFPs ports name (T_SFPName::SFP1, T_SFPName::SFP2 or T_SFPName::SFP3). Maximum, we have three SFP port to receive images data.
	* \return void
    */
    void set_SFP_network_config(T_UDPConfig udp_config, T_SFPName sfp_name);

    /**
    * \fn void get_all_images(char ** data_images_table, T_AcquisitionMode acq_mode, std::size_t images_nb, std::size_t & frame_number)
	...
	* \return void
    */
    void get_all_images(char ** data_images_table, T_AcquisitionMode acq_mode, std::size_t images_nb, std::size_t & frame_number);
    ...
    */
    ...
    /**
    * \fn void ordering_data(yat::Socket::Data img_raw_Two_chip[], char **data_images_table, T_AcquisitionMode acq_mode, std::size_t images_nb, std::size_t & frame_number)
	...
	* \return void
    */
    void ordering_data(yat::Socket::Data img_raw_Two_chip[], char **data_images_table, T_AcquisitionMode acq_mode, std::size_t images_nb, std::size_t & frame_number);

    /**
    * \fn std::size_t get_frame_number_for_2_counters(T_AcquisitionMode acq_mode)
    * \brief get the frames number for two counters
    * \param acq_mode : acquisition mode. This parameter determine  how many bits per pixel
	* \return the frames number
    */
    std::size_t get_frame_number_for_2_counters(T_AcquisitionMode acq_mode);
    ...
    * this function call the get_all_images with ((images_nb = 1) && (acq_mode = ufxclib::T_AcquisitionMode::software_raw))
